...
export const ConnectionModal: React.FC<ConnectionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/40 backdrop-blur-xs">
      <div className="bg-white border-t sm:border border-slate-200 rounded-t-3xl sm:rounded-2xl max-w-lg w-full p-5 sm:p-6 shadow-2xl relative animate-in slide-in-from-bottom-5 duration-200">
        <button
          onClick={onClose}
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-600 space-y-1.5 font-medium">
            <div className="flex items-center gap-1.5 font-bold text-slate-800">
              <FileText className="w-4 h-4 text-emerald-600" /> Script SQL
            </div>
            <p>
              Execute <code className="text-emerald-700 bg-emerald-50 px-1 py-0.5 rounded border border-emerald-200 font-bold">supabase_setup.sql</code> no SQL Editor do Supabase.
            </p>
          </div>
          ...
          <div className="pt-2 flex items-center justify-between gap-3">
            <button
              type="button"
              onClick={handleClear}
              className="px-3 py-3 text-xs text-slate-500 hover:text-rose-700 hover:bg-rose-50 rounded-xl transition-colors font-bold min-h-[44px]"
            >
              Modo Demo
            </button>
            ...
            <button
              type="submit"
              className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold px-5 py-3 rounded-xl shadow-xs transition-all flex items-center justify-center space-x-2 text-xs min-h-[44px]"
            >
              {savedSuccess ? (
                <>
