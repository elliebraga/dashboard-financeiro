...
import { Calendar, ChevronRight, TrendingUp, TrendingDown, DollarSign, X, Tag, FileText, PieChart, Sparkles } from 'lucide-react';
...
export const MonthlyComparator: React.FC<MonthlyComparatorProps> = ({ transactions, categories }) => {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md border border-slate-200/80 rounded-3xl p-4 sm:p-6 shadow-xs space-y-4">
        ...
        {summaries.map((m) => {
          ...
          return (
            <div
              ...
              className="bg-slate-50/80 hover:bg-white border border-slate-200/80 hover:border-emerald-300 rounded-3xl p-4 transition-all cursor-pointer group shadow-2xs relative"
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-0.5 rounded-full flex items-center gap-0.5 group-hover:bg-emerald-600 group-hover:text-white transition-colors">
      ...
      {/* MODAL DE DETALHES DO MÊS SELECIONADO (DESIGN FOFO ARREDONDADO) */}
      {selectedMonth && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/30 backdrop-blur-xs">
          <div className="bg-white border-t sm:border border-slate-200/80 rounded-t-3xl sm:rounded-3xl max-w-2xl w-full max-h-[90vh] flex flex-col p-5 sm:p-6 shadow-2xl relative animate-in slide-in-from-bottom-5 duration-200 overflow-hidden">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-200 shrink-0">
              ...
              <button
                ...
                className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-2xl transition-colors min-h-[40px] min-w-[40px] flex items-center justify-center"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 py-4 space-y-6 pr-1">
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-4">
                <div className="bg-slate-50/80 p-3 rounded-2xl border border-slate-200 text-center">
                  <span className="block text-[10px] font-bold text-slate-400 uppercase">Receitas</span>
                ...
                {getCategoryBreakdown(selectedMonth.transactions).length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {getCategoryBreakdown(selectedMonth.transactions).map((cat) => (
                      <div key={cat.name} className="flex items-center justify-between bg-slate-50/80 p-2.5 rounded-2xl border border-slate-200 text-xs">
                  ...
                  {selectedMonth.transactions
                    ...
                    .map((tx) => {
                      ...
                      return (
                        <div
                          key={tx.id}
                          className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-2xl hover:bg-slate-50 transition-colors"
                        >
                          <div className="flex items-center space-x-2.5">
                            <div className={`p-1.5 rounded-xl border ${isInc ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-rose-50 text-rose-600 border-rose-200'}`}>
            ...
            <div className="pt-3 border-t border-slate-200 flex justify-end shrink-0">
              <button
                onClick={() => setSelectedMonth(null)}
                className="px-5 py-2.5 bg-slate-900 text-white font-bold text-xs rounded-2xl shadow-xs hover:bg-slate-800 transition-colors min-h-[44px]"
              >
                Fechar Detalhes
              </button>
            </div>
