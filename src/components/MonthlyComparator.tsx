...
export const MonthlyComparator: React.FC<MonthlyComparatorProps> = ({ transactions, categories }) => {
  ...
  return (
    <div className="bg-white border border-slate-300 rounded-2xl p-4 sm:p-6 shadow-xs space-y-4">
        ...
        {summaries.map((m) => {
          ...
          return (
            <div
              ...
              className="bg-slate-50 hover:bg-slate-100 border border-slate-300 hover:border-emerald-500 rounded-2xl p-4 transition-all cursor-pointer group shadow-2xs relative"
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs font-extrabold text-emerald-800 bg-emerald-50 border border-emerald-300 px-2 py-0.5 rounded-full flex items-center gap-0.5 group-hover:bg-emerald-700 group-hover:text-white transition-colors">
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <div className="bg-white p-2.5 rounded-xl border border-slate-300">
                  <span className="block text-[10px] font-extrabold text-slate-700 uppercase">Receitas</span>
                  <span className="text-xs font-extrabold text-emerald-800 flex items-center gap-0.5">
                    <TrendingUp className="w-3 h-3" /> {formatCurrency(m.income)}
                  </span>
                </div>

                <div className="bg-white p-2.5 rounded-xl border border-slate-300">
                  <span className="block text-[10px] font-extrabold text-slate-700 uppercase">Despesas</span>
                  <span className="text-xs font-extrabold text-rose-800 flex items-center gap-0.5">
              ...
              <div className="flex items-center justify-between pt-1 border-t border-slate-300">
                <span className="text-[11px] font-extrabold text-slate-700 uppercase">Saldo do Mês</span>
                <span className={`text-sm font-extrabold ${isPositive ? 'text-slate-900' : 'text-rose-800'}`}>
      ...
      {/* MODAL DE DETALHES DO MÊS SELECIONADO */}
      {selectedMonth && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/60 backdrop-blur-xs">
          <div className="bg-white border-t sm:border border-slate-300 rounded-t-3xl sm:rounded-2xl max-w-2xl w-full max-h-[90vh] flex flex-col p-5 sm:p-6 shadow-2xl relative animate-in slide-in-from-bottom-5 duration-200 overflow-hidden">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-300 shrink-0">
              ...
              <button
                ...
                className="p-2 text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-colors min-h-[40px] min-w-[40px] flex items-center justify-center"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 py-4 space-y-6 pr-1">
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-4">
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-300 text-center">
                  <span className="block text-[10px] font-extrabold text-slate-700 uppercase">Receitas</span>
                ...
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-300 text-center">
                  <span className="block text-[10px] font-extrabold text-slate-700 uppercase">Saldo</span>
                  <span className={`text-sm sm:text-base font-extrabold ${selectedMonth.balance >= 0 ? 'text-slate-900' : 'text-rose-800'}`}>
              ...
              <div>
                ...
                {getCategoryBreakdown(selectedMonth.transactions).length === 0 ? (
                  <p className="text-xs text-slate-600 font-semibold italic">Nenhuma despesa registrada neste mês.</p>
                ) : (
                  <div className="space-y-2">
                    {getCategoryBreakdown(selectedMonth.transactions).map((cat) => (
                      <div key={cat.name} className="flex items-center justify-between bg-slate-50 p-2.5 rounded-xl border border-slate-300 text-xs">
                  ...
                  {selectedMonth.transactions
                    ...
                    .map((tx) => {
                      ...
                      return (
                        <div
                          key={tx.id}
                          className="flex items-center justify-between p-3 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 transition-colors"
                        >
                          <div className="flex items-center space-x-2.5">
                            <div className={`p-1.5 rounded-lg border ${isInc ? 'bg-emerald-50 text-emerald-800 border-emerald-300' : 'bg-rose-50 text-rose-800 border-rose-300'}`}>
                          ...
                          <span className={`text-xs font-extrabold ${isInc ? 'text-emerald-800' : 'text-rose-800'}`}>
                            {isInc ? '+ ' : '- '}
            ...
            {/* Footer do Modal */}
            <div className="pt-3 border-t border-slate-300 flex justify-end shrink-0">
              <button
                onClick={() => setSelectedMonth(null)}
                className="px-5 py-2.5 bg-slate-900 text-white font-extrabold text-xs rounded-xl shadow-xs hover:bg-slate-800 transition-colors min-h-[44px]"
              >
                Fechar Detalhes
              </button>
            </div>
